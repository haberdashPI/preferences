...
keybindings: {
    ...
    // NOTE: this will need to be a special case of `::using::` that checks for `doc` and moves it outwards
    "::using::selection-utilities.moveBy": {
        // word-like
        w:     { unit: "subword", boundary: "start", select:      true, value: ' (__count || 1)', 
                 doc: { kind: "select", label: "subwrd →", detail: "next subword (camel/snake case)" } },
        uw:    { unit: "subword", boundary: "start", selectWhole: true, value: ' (__count || 1)', 
                 doc: { kind: "select", label: "subwrd →", detail: "select entire subword with and trailing whitespace (camel/snake case)" } },
        ue:    { unit: "word",    boundary: "both",  selectWhole: true, value: ' (__count || 1)', 
                 doc: { kind: "select", label: "in word →", detail: "select entire word (no whitespace)" } },
        W:     { unit: "word",    boundary: "start", select:      true, value: ' (__count || 1)', 
                 doc: { kind: "select", label: "word →", detail: "next word"} },
        uW:    { unit: "word",    boundary: "start", selectWhole: true, value: ' (__count || 1)', 
                 doc: { kind: "select", label: "word →", detail: "select entire word and trailing whitespace"} },
        uE:    { unit: "word",    boundary: "both",  selectWhole: true, value: '-(__count || 1)', 
                 doc: { kind: "select", label: "in word ←", detail: "select previous word (no whitespace)" } },
        e:     { unit: "subword", boundary: "end",   select:      true, value: ' (__count || 1)', 
                 doc: { kind: "select", label: "word end →", detail: "next word end" } },
        b:     { unit: "subword", boundary: "start", select:      true, value: '-(__count || 1)', 
                 doc: { kind: "select", label: "subwrd ←", detail: "previous subword (came/snake case)" } },
        ub:    { unit: "subword", boundary: "start", selectWhole: true, value: '-(__count || 1)', 
                 doc: { kind: "select", label: "subwrd ←", detail: "select previous subword and trailing whitespace (came/snake case)" } },
        B:     { unit: "word",    boundary: "start", select:      true, value: '-(__count || 1)', 
                 doc: { kind: "select", label: "word ←", detail: "previous word" } },
        uB:    { unit: "word",    boundary: "start", selectWhole: true, value: '-(__count || 1)', 
                 doc: { kind: "select", label: "word ←", detail: "select previous word and trailing whitespace" } },
        E:     { unit: "word",    boundary: "end",   select:      true, value: '-(__count || 1)', 
                 doc: { kind: "select", label: "word end ←", detail: "previous word end" } },
        "'w":  { unit: "WORD",    boundary: "start", select:      true, value: " (__count || 1)", 
                 doc: { kind: "select", label: "WORD →", detail: "next WORD; e.g. contiguous non-whitespace region"} },
        "'b":  { unit: "WORD",    boundary: "start", select:      true, value: "-(__count || 1)", 
                 doc: { kind: "select", label: "WORD ←", detail: "previous WORD; e.g. contiguous non-whitespace region"} },
        "'e":  { unit: "WORD",    boundary: "end",   select:      true, value: "-(__count || 1)", 
                 doc: { kind: "select", label: "WORD end →", detail: "to end of WORD; e.g. contiguous non-whitespace region"} },
        "u'w": { unit: "WORD",    boundary: "start", selectWhole: true, value: " (__count || 1)", 
                 doc: { kind: "select", label: "WORD →", detail: "select entire WORD and trailing whitespace; a WORD is a contiguous non-whitespace region" } },
        "u'e": { unit: "WORD",    boundary: "both",  selectWhole: true, value: " (__count || 1)", 
                 doc: { kind: "select", label: "in WORD →", detail: "select entire WORD; a WORD is a contiguous non-whitespace region" } },
        "u'b": { unit: "WORD",    boundary: "start", selectWhole: true, value: "-(__count || 1)", 
                 doc: { kind: "select", label: "WORD →", detail: "select previous entire WORD; a WORD is a contiguous non-whitespace region" } },
